...
export default function Navbar() {
  ...
  const isActive = (path) => {
    return location.pathname === path ? 'bg-violet-900/20 text-violet-400' : '';
  };
    ...
    >
      <div className="max-w-7xl mx-auto flex justify-between items-center">
        ...
        <div className="flex gap-6">
          ...
          <Link to="/geforce-now" className={`nav-link ${isActive('/geforce-now')}`}>GeForce NOW</Link>
          <Link to="/stremio" className={`nav-link ${isActive('/stremio')}`}>Stremio</Link>
